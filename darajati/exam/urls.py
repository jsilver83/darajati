...
urlpatterns = [
    ...
    re_path(r'^settings/(?P<fragment_id>[0-9]+)/$',
            views.ExamSettingsView.as_view(),
            name='settings'
            ),

    re_path(r'^shifts/(?P<fragment_id>[0-9]+)/$',
            views.ExamShiftsView.as_view(),
            name='shifts'
            ),

    re_path(r'^rooms/(?P<fragment_id>[0-9]+)/$',
            views.ExamRoomsView.as_view(),
            name='exam_rooms'
            ),

    re_path(r'^markers/(?P<fragment_id>[0-9]+)/$',
    ...
    re_path(r'^unaccepted/(?P<fragment_id>[0-9]+)/$',
            views.UnacceptedStudentMarksView.as_view(),
            name='unaccepted_markers'
            ),

    re_path(r'^markers-listing/(?P<fragment_id>[0-9]+)/$',
            views.CoordinatorMarkersListing.as_view(),
            name='coordinator_markers_listing'
            ),
]
