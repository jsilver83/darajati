...
from enrollment.web_service_utils import get_section
...
class AttendanceView(FormSetView):
    ...
    def get_initial(self, **kwargs):
        if self.kwargs.get('crn'):
            self.crn = self.kwargs.get('crn')
            self.section = get_section(semester_code='201710-SH', crn=self.crn)
            return Attendance.get_section_enrollment(self.crn, today())

    def get_context_data(self, **kwargs):
        context = super(AttendanceView, self).get_context_data(**kwargs)

        context.update({
            'current_periods': ScheduledPeriod.objects.filter(section=self.section['sec_code'])
        })
        return context
